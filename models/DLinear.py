...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec,text_emb, y_future, compute_aux):
        ...
        if self.use_text and text_emb is not None:
            y_tir_scalar, kappa, params = self.tical_plugin._text_induced_response(text_emb, enc_out.size(1))
            
            aux = {}
            if compute_aux and y_future is not None:
                aux = self.tical_plugin._aux_losses(kappa, params, y_future, enc_out.size(1))
            
            y_hat, gate = self.tical_plugin._fuse(enc_out, y_tir_scalar, x_enc, text_emb)
            aux.update({
