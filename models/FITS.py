...
class Model(nn.Module):
    ...
    def forward(self, x, x_mark_enc, x_dec, x_mark_dec, mask=None,text_emb: torch.Tensor = None,y_future: torch.Tensor = None, compute_aux: bool = False):
        # RIN
        ...
        if self.use_text and text_emb is not None:
            y_tir_scalar, kappa, params = self.tical_plugin._text_induced_response(text_emb, xy.size(1))
            
            aux = {}
            if compute_aux and y_future is not None:
                aux = self.tical_plugin._aux_losses(kappa, params, y_future, xy.size(1))
            
            y_hat, gate = self.tical_plugin._fuse(xy, y_tir_scalar, x, text_emb)
            aux.update({
            ...
            return y_hat[:, -self.pred_len:, :], aux 
        return xy,None
