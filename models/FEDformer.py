...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec,text_emb, y_future, compute_aux):
        ...
        if self.use_text and text_emb is not None:
            TICAL_infer_start_time = time.time()    
            ...
            y_hat, gate = self.tical_plugin._fuse(dec_out, y_tir_scalar, enc_int, text_emb)
            aux.update({
            ...
            TICAL_infer_end_time = time.time()
            TICAL_infer_time = TICAL_infer_end_time - TICAL_infer_start_time  # 计算单次推理时间
            print(f"TICAL Inference Time during training: {TICAL_infer_time:.4f} seconds")  
